...
{
    IOrasClient CreateOrasClient(string? authToken = null);
    IEncryptor CreateEncryptor();
    IBackupEngine CreateBackupEngine(string? authToken = null);
    IRestoreEngine CreateRestoreEngine(string? authToken = null);
    IChunkEngine CreateChunkEngine(string? authToken = null);
...
public class DefaultServiceFactory : IServiceFactory
{
    ...
    public virtual IOrasClient CreateOrasClient(string? authToken = null) =>
        new HttpOrasClient(CreateHttpClient(), _loggerFactory.Value.CreateLogger<HttpOrasClient>(), authToken);
    ...
    public virtual IChunkEngine CreateChunkEngine(string? authToken = null) =>
        new ChunkEngine(CreateOrasClient(authToken), CreateEncryptor(), _loggerFactory.Value.CreateLogger<ChunkEngine>());

    public virtual IBackupEngine CreateBackupEngine(string? authToken = null) =>
        new BackupEngine(new DeltaTracker(), CreateChunkEngine(authToken), CreateOrasClient(authToken),
            CreateEncryptor(), _loggerFactory.Value.CreateLogger<BackupEngine>());

    public virtual IRestoreEngine CreateRestoreEngine(string? authToken = null) =>
        new RestoreEngine(CreateOrasClient(authToken), CreateEncryptor(), _loggerFactory.Value.CreateLogger<RestoreEngine>());
